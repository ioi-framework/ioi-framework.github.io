...
category:     Temporal
status:       Validated
description:  "Detects timestamp forging by comparing the LNK shortcut creation time against the $SI timestamp for the same MFT entry, corroborated by $FN agreement with the LNK value."
artifacts:
  - LNK
  - $MFT
invariant:    "The LNK shortcut creation time must agree with the MFT $FN (0x30) creation timestamp for the same entry at second-level precision while diverging from the MFT $SI (0x10) timestamp."
...
notes:        "The strict variant uses STRSTARTS for Virtuoso-safe second-precision comparison. A permissive variant (ioi-011-permissive.rq) omits the $FN corroboration check for higher recall."
---
