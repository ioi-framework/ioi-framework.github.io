...
category:     Temporal
status:       Validated
description:  "Detects timestamp forging of Office documents by joining embedded Office core.xml metadata against the MFT $SI creation timestamp for the same file path. Fires when the embedded creation time predates filesystem creation time by at least one minute."
artifacts:
  - Office core.xml
  - $MFT
invariant:    "The Office XML embedded creation minute must not predate the MFT $SI creation minute for the same file path."
...
A document's embedded Office XML creation minute must not predate the filesystem `$SI` creation minute for the same file path.

## Contradiction condition

The embedded Office XML creation minute is earlier than the MFT `$SI` creation minute, indicating the filesystem timestamp was pushed forward after the document was created. Sub-minute skew, such as `14:26:00Z` versus `14:26:37Z`, is ignored.
...
The Office XML graph is very small compared with the MFT graph. The normalized `filePath` join eliminates the earlier filename-only Cartesian-product problem, and the `GROUP BY + MIN()` pattern keeps the result stable when residual duplicate MFT matches exist. Timestamp comparison is intentionally minute-precision to avoid false positives from normal sub-minute write skew.
