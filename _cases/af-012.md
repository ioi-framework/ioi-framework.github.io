...
category:    "Temporal"
status:      Validated
description: "Filesystem timestamps forged via timestomper.exe; embedded Office core.xml metadata disagrees with the forged MFT $SI timestamp in a cross-graph filePath join."
artifacts:
  - Office core.xml
  - $MFT
...
- MFT `$STANDARD_INFORMATION` (0x10) shows the forged creation timestamp
- `xmlCreated != mftSiCreated` at minute precision for the same normalized `filePath` indicates timestomping
